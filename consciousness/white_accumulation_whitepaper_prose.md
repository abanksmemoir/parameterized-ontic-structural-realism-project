...
**Michael Abramson**
*April 2026*
*Working Paper — v1 (Prose Edition)*
...
The term originated in a dream on the night of April 10–11, 2026, following a long conversation about time symmetry, language models, and the relationship between thermodynamic irreversibility and the hard problem of consciousness. In the dream, a lecturer—name unclear, something like "Hoyt"—presented the concept at what appeared to be an information theory talk. The term does not exist in the literature. The concept it points to, however, is precise.
...
The handshake is the central concept of this framework. It is not a signal, not backward causation, and not a computational property. It is a constraint on admissible trajectories. A state is admissible only if it belongs to a globally self-consistent trajectory across a nontrivial temporal interval.
...
This framework provides the missing rule. A subject is the maximal set of degrees of freedom that jointly satisfy temporal closure. This has several immediate consequences. Arbitrary aggregates such as a brain and a coffee cup are excluded: they share no trajectory closure. Proper subsets of a conscious system are excluded: they do not independently satisfy closure. Overlapping subjects are impossible: closure defines non-overlapping maximal units. The subject is not a spatial aggregate. It is a dynamically defined, temporally bounded process.

---

## 6. Formal Structure

### 6.1 The Closure Functional

The formal apparatus centers on a closure functional—a quantity that measures how far a given trajectory deviates from temporal self-consistency. A system is closed over an interval when four conditions are jointly met.

First, admissibility: the realized trajectory must satisfy global consistency to within a threshold. Second, non-genericity: most nearby perturbations of the trajectory must violate closure. The trajectory sits in a narrow admissible class, not a generic region of state space. Third, non-factorizability: the whole system must pass while its parts, evaluated independently, fail. This is the formal expression of binding. Fourth, boundary maximality: adding degrees of freedom to the system must degrade closure, not improve it. This prevents arbitrary enlargement and solves the boundary problem.

### 6.2 Decomposition

The closure functional can be decomposed into three components. A persistence term measures deviation from noise and structural continuity across the interval. A decomposability penalty measures whether the trajectory can be split into independently valid sub-trajectories. And a bidirectional consistency term measures whether the middle portion of the trajectory is jointly licensed by what comes before and what comes after—whether forward inference from the past and backward inference from the future converge on the same present. This last term is the formal home of the handshake.

### 6.3 Toy Model

The simplest illustration uses three time slices: past, present, and future. A closure functional combines three penalties: deviation of the present from what the past would predict, deviation of the future from what the present would predict, and deviation of the present from the midpoint between past and future (a minimal global consistency condition).

Minimizing this functional yields a present state that is not generated from the past alone but selected as a compromise between past constraint, future constraint, and whole-trajectory consistency. A tunable parameter controls how much weight the global constraint carries. When this parameter is zero, the system is purely forward-evolving. As it increases, the system becomes increasingly closure-dominated—its present becomes intelligible only as part of a globally consistent trajectory.

The model extends naturally to multi-component systems. When two coupled variables jointly satisfy closure but neither does independently, binding emerges directly from the formalism.

---

## 7. The Closure Dominance Parameter

The framework requires a dimensionless quantity that measures how much a system's dynamics resist purely forward-evolving description. The Closure Dominance Parameter is defined as the fractional improvement in descriptive accuracy gained by introducing temporal closure constraints, compared to the best possible retarded-only (forward-evolving) model.

When this parameter is near zero, the system is fully characterizable by forward evolution; adding closure constraints contributes nothing. When it approaches one, the system's organization becomes intelligible only when modeled as a temporally closed trajectory. The hypothesis is that consciousness lives in the high end of this parameter.

A composite mind-likeness parameter can be constructed by multiplying closure dominance by a measure of non-factorizability. High values on both dimensions identify candidate conscious systems: strong closure, strong unity.

---

## 8. Identity, Distance, and Distinction Between Minds
...
### 8.2 Distinction Between Minds

Two trajectories belong to the same subject if and only if they can be jointly embedded in a single non-factorizable, temporally closed trajectory. Otherwise they are distinct subjects. This single rule handles personal identity over time, spatial distinction between persons, duplication (copies split at the first moment of trajectory incompatibility), and merging (subjects unify when they form a joint closure).

### 8.3 Distance Between Minds

The distance between two minds can be defined as the minimal closure cost of treating them as one subject—the cost of finding a single admissible trajectory that contains both. Small distance means nearly the same subject. Large distance means clearly distinct. Infinite or undefined distance means no admissible unification exists.

---

## 9. Physics and Time Symmetry

### 9.1 Boundary-Conditioned Systems in Physics
...
### 9.2 Wheeler–Feynman and Free-Electron Lasers
...
This is not merely an interpretive curiosity. In certain highly coherent, weak-absorption regimes—notably the radiation of coherently oscillating electrons in free-electron lasers—conventional retarded-only electrodynamics encounters difficulties. Niknejadi, Madey, and Kowalczyk (2015, *Physical Review D*) found it necessary to revive the Wheeler–Feynman assumption of equal advanced and retarded amplitudes to resolve discrepancies in the coherent radiation problem. They also proposed experimental detection of advanced effects by reducing absorber coupling.

The relevance to this framework is not that the brain is a free-electron laser. It is that physics already admits regimes in which retarded-only descriptions are not merely cumbersome but structurally inadequate. If such regimes exist, it is legitimate to ask whether other complex systems—particularly those characterized by dense nonlinear coupling, massive recurrence, and extreme sensitivity to boundary conditions—might also belong to the same descriptive class.

### 9.3 The Strong Structural Claim

The claim is not that advanced electromagnetic signals propagate in neural tissue. It is stronger and more general. In a sufficiently complex, nonlinear, recursively coupled system, a purely forward-evolution description is not just inconvenient—it is structurally inadequate as a generalizable explanatory framework. Such systems cannot be cleanly decomposed into forward-propagating causal steps. Their admissible trajectories are defined by global consistency, not local generation. The advanced Green's function, in this context, is not a literal signal but the mathematical signature that the system belongs to a class whose dynamics are inherently boundary-conditioned.

---

## 10. Neural Closure Regimes
...
**Weakly coupled systems** such as diffusion processes and simple circuits exhibit no closure because trajectory selection is weak. They are fully characterizable by forward evolution.

**Pure noise** has no structure to constrain. Closure is undefined.

**Rigid order**—crystals, static fields—satisfies closure trivially, because the admissible trajectory space has collapsed to a degenerate subset. There is no meaningful distinction between possible and actual trajectories. This is not consciousness; it is death by over-constraint.

**Pathological synchrony** in seizures produces high coherence and strong coupling, but extreme reduction of dynamical dimensionality. The system collapses into a degenerate attractor, resembling rigid order more than genuine closure.

**Deep sleep and anesthesia** preserve persistent neural activity and structured oscillations, but long-range temporal integration collapses. Recursive coupling weakens. The system becomes locally consistent but globally unconstrained. Closure is lost through under-constraining.

**Transient coherent events**—brief bursts of structured activity—may exhibit local coherence but lack the temporal thickness required for closure.

**Large language models** exhibit local coherence and high generative flexibility but zero intrinsic closure. They simulate trajectories without instantiating temporally closed ones. They borrow structure from pre-compressed training data rather than maintaining it under real constraint.
