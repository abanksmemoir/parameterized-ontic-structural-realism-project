"""
Cross-theory structural comparison (V2).

Identifies shared subgraphs, fork points, and theory-specific branches
between different theory-graphs (e.g., SM vs GR).

V2 changes:
  - Dependency-type-aware comparison: can filter by logical/conventional/contingent
  - Parameter-type-aware signatures
  - Temporal chain comparison
"""

from typing import Dict, List, Set, Any, Optional, Tuple
from .schema import TheoryGraph, Node, Dependency
...
def _compute_depth(graph: TheoryGraph, node_id: str) -> int:
    ...
    def dfs(nid: str) -> int:
        ...
        if not node.dependencies:
            ...
            result = 1 + max(dfs(dep.on) for dep in node.dependencies)
        memo[nid] = result
        return result
...
def identify_fork_path(
    ...
    all_roots = [
        nid for nid in diff.shared_nodes
        if not graph1.nodes[nid].dependencies
    ]
    ...
    while queue:
        ...
        for node_id in graph1.nodes:
            ...
            if (
                current in node.dependency_ids()
...
def extract_theory_branch(
    ...
    while queue:
        ...
        for other_id, other_node in graph.nodes.items():
            if current in other_node.dependency_ids() and other_id not in descendants:
    ...
    for node_id in descendants:
        ...
        deps_in_branch = [
            dep for dep in node.dependencies if dep.on in descendants
        ]
        new_node = Node(
            id=node.id,
    ...
    try:
        ...
        pass  # Branch root may have external dependencies
...
def dependency_type_summary(graph: TheoryGraph) -> Dict[str, int]:
    """Count dependencies by type across the entire graph."""
    counts = {"logical": 0, "conventional": 0, "contingent": 0}
    for node in graph.nodes.values():
        for dep in node.dependencies:
            if dep.dependency_type in counts:
                counts[dep.dependency_type] += 1
    return counts


def parameter_type_summary(graph: TheoryGraph) -> Dict[str, int]:
    """Count parameters by subtype across the entire graph."""
    counts = {t: 0 for t in ["axiom", "structural", "kinematic", "dynamical", "boundary", "convention"]}
    for node in graph.nodes.values():
        for param in node.parameters:
            if param.parameter_type in counts:
                counts[param.parameter_type] += 1
    return counts
